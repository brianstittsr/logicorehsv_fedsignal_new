...
export function UserProfileProvider({ children }: { children: ReactNode }) {
  ...
  // Handle demo login (HBCU1) - check sessionStorage first
  useEffect(() => {
    if (typeof window === "undefined") return;

    const demoLogin = sessionStorage.getItem("fedsignal_demo_login");
    const demoUsername = sessionStorage.getItem("fedsignal_username");
    const sessionRole = sessionStorage.getItem("svp_user_role");
    const sessionName = sessionStorage.getItem("svp_user_name");

    if (demoLogin === "true") {
      console.log("Demo login detected:", demoUsername);
      setIsAuthenticated(true);
      setIsLoading(false);
      setProfile((prev) => ({
        ...prev,
        id: demoUsername || "demo-user",
        email: `${demoUsername}@demo.local`,
        firstName: sessionName?.split(" ")[0] || "Demo",
        lastName: sessionName?.split(" ").slice(1).join(" ") || "User",
        role: (sessionRole as UserProfile["role"]) || "team_member",
        updatedAt: new Date().toISOString(),
      }));
    }
  }, []);
  ...
  useEffect(() => {
    ...
    // Skip Firebase auth if demo login is active
    if (typeof window !== "undefined" && sessionStorage.getItem("fedsignal_demo_login") === "true") {
      console.log("Demo login active, skipping Firebase auth check");
      return;
    }
