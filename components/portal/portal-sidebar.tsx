...
import { useUserSwitcher } from "@/contexts/user-switcher-context";
import {
  Sidebar,
  SidebarContent,
  SidebarFooter,
...
export function PortalSidebar() {
  ...
  const { hasVisibility } = useUserSwitcher();
  ...
  const filterNavItems = (items: NavItem[]) => {
    return items.filter((item: NavItem) => {
      ...
      // Check role-based visibility from Firebase settings
      ...
      // Check user switcher visibility settings (for HBCU users)
      // Extract feature name from href for visibility check
      const featureName = item.href.split('/').pop() || '';
      const hasFeatureVisibility = hasVisibility(featureName);
      
      return !isHiddenForRole && !isGloballyHidden && hasFeatureVisibility;
    });
  };
