...
import { cn, formatCurrency, getDeadlineColor, getMatchColor } from '@/lib/utils'
...
function KPICard({ 
  ...
  const colorMap = {
    radar: { text: 'text-radar', bg: 'bg-radar', after: 'bg-radar' },
    green: { text: 'text-green-700', bg: 'bg-green-600', after: 'bg-green-700' },
    amber: { text: 'text-amber-700', bg: 'bg-amber-600', after: 'bg-amber-700' },
    red: { text: 'text-red-700', bg: 'bg-red-600', after: 'bg-red-700' },
    uni: { text: 'text-[var(--uni-primary)]', bg: 'bg-[var(--uni-primary)]', after: 'bg-[var(--uni-primary)]' },
  }
  
  const colors = colorMap[color]
  
  return (
    <div className="relative bg-white border border-[#e2e8f0] rounded-lg shadow-[0_1px_3px_rgba(0,0,0,0.04)] p-5 overflow-hidden hover:shadow-[0_3px_10px_rgba(0,0,0,0.08)] transition-shadow">
      <div className={cn("absolute bottom-0 left-0 right-0 h-[3px]", colors.after)} />
      <div className="text-[11px] font-semibold uppercase tracking-wide text-[#64748b] mb-1.5">
        {label}
      </div>
      <div className="font-display text-[28px] font-extrabold text-[#0f172a] leading-none">
        {value}
      </div>
      <div className={cn(
...
function OpportunityRow({
  ...
  agency: string
  solicitation: string
  tags: { label: string; variant: 'hbcu' | 'cyber' | 'contract' | 'grant' | 'ai' | 'defense' | 'stem' }[]
  amount: string
  amountLabel: string
  deadline: string
  ...
  return (
    <div className="grid grid-cols-[48px_1fr_auto_auto_100px] items-center gap-4 px-5 py-3.5 border-b border-[#f1f5f9] hover:bg-[#f8faff] transition-colors cursor-pointer">
      <div className={cn(
        "w-11 h-11 rounded-full flex items-center justify-center font-mono text-xs font-semibold border-2",
        matchStyles[matchColor as keyof typeof matchStyles]
      )}>
        {match}
      </div>
      <div>
        <div className="text-sm font-semibold text-[#0f172a] mb-1">{title}</div>
        <div className="font-mono text-[11px] text-[#64748b] mb-2">{agency} · {solicitation}</div>
      ...
      <div className={cn(
        ...
        deadlineStyles[deadlineColor as keyof typeof deadlineStyles]
      )}>
  ...
  return (
    <div className="mb-3">
      <div className="flex justify-between mb-1.5 text-[13px]">
        ...
        <span className="font-mono text-radar font-semibold">{value}</span>
      </div>
      <div className="h-1.5 bg-[var(--bg-3)] rounded-sm overflow-hidden">
        <div 
...
export default function DashboardPage() {
  return (
    <div className="flex h-screen overflow-hidden bg-[#f4f6f9]">
      <Sidebar universityId="tuskegee" />
      
      <div className="flex-1 flex flex-col overflow-hidden">
        <Topbar title="Command Center" universityName="Tuskegee University" />
        
        <main className="flex-1 overflow-y-auto p-6">
          {/* Phase Banner */}
          <div className="flex items-center gap-3 px-4 py-3 mb-5 bg-gradient-to-r from-blue-50 to-green-50 border border-blue-200 rounded-lg">
            <span className="bg-radar text-white text-[11px] font-bold px-2.5 py-1 rounded whitespace-nowrap">
              Phase 2 Active
            </span>
            <span className="text-[13px] text-[#334155]">
              <strong>Leadership Intelligence + Partnership Marketplace</strong> now live. 3 consortiums pending your review. FAMU trial expires Apr 10.
            </span>
          </div>
          ...
          <div className="grid grid-cols-5 gap-3 mb-6">
            ...
            <KPICard 
              label="Deadlines 30d" 
              value="6" 
              subtext="3 urgent — action needed"
              trend="down"
              color="red"
            />
          </div>
          ...
          <div className="grid grid-cols-[2fr_1fr] gap-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <Card accent="green">
                  <div className="p-5">
                    ...
                    <FundingBar name="Cybersecurity" value="$218M" percent={100} color="var(--radar)" />
                    <FundingBar name="Defense R&D" value="$320M" percent={85} color="var(--green)" />
                    <FundingBar name="AI / ML" value="$142M" percent={60} color="var(--purple)" />
                    <FundingBar name="Energy / Grid" value="$185M" percent={70} color="var(--teal)" />
                    <FundingBar name="STEM Education" value="$96M" percent={40} color="var(--amber)" />
                  </div>
                </Card>
            ...
            <div className="space-y-4">
              ...
              <Card accent="uni">
                <div className="p-5">
                  <div className="flex items-center gap-2 text-[11px] font-semibold uppercase tracking-wide mb-4" style={{ color: 'var(--uni-primary)' }}>
                    <span>Institution Score</span>
                    <div className="flex-1 h-px bg-[var(--border-color)]" />
                  </div>
                  <div className="text-center py-2.5">
                    <div className="font-display text-[52px] font-extrabold leading-none" style={{ color: 'var(--uni-secondary)' }}>
                      76
                    </div>
                    ...
                    {[
                      ...
                      <div 
                        ...
                        className={cn(
                          ...
                          item.color === 'red' ? 'bg-red-50/50 border-red-200' : 
                          item.color === 'amber' ? 'bg-amber-50/50 border-amber-200' :
                ...
                <div className="p-5">
                  ...
                  <div className="flex gap-1 flex-wrap mb-2.5">
                    {['Tuskegee', 'Howard', 'FAMU', 'Morgan State'].map((m) => (
                      <span key={m} className="text-[11px] px-2 py-0.5 bg-[var(--bg-2)] border border-[var(--border-color)] rounded-full text-[#64748b]">
                        {m}
                      </span>
                    ))}
                  </div>
                  ...
                  <Button variant="radar" size="sm" className="w-full text-xs">
                    Open Workspace →
                  </Button>
                </div>
              </Card>
