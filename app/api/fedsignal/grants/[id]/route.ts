...
export async function GET(
  ...
  try {
    ...
    const grant = await getDocument(FSCOLLECTIONS.GRANTS, id);
...
export async function PUT(
  ...
  try {
    ...
    const grant = await updateDocument(FSCOLLECTIONS.GRANTS, id, validationResult.data);
...
export async function DELETE(
  ...
  try {
    const { id } = await params;
    await deleteDocument(FSCOLLECTIONS.GRANTS, id, true);
